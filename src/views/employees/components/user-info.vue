<template>
  <div class="user-info">
    <el-row type="flex" justify="end">
      <el-tooltip class="item" effect="dark" content="打印个人信息" placement="right-end">
...
export default {
  ...
  methods: {
    ...
    //  更新上半部分
    async saveUser() {
      const fileList = this.$refs.image.file
      if (fileList.some(item => !item.upload)) {
        this.$message.error('还未上传完毕')
        return
      }
    ...
    //  更新下半部分
    async savePersonal() {
      const fileList = this.$refs.empyessRef.file
      if (fileList.some(item => !item.upload)) {
        this.$message.error('还未上传完毕')
        return
      }
